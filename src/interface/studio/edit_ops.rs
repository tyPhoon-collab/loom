...
impl StudioApp {
    ...
    pub(super) fn adjust_current_template_call_repeat(&mut self, delta: i32) -> Result<()> {
        let Some(call) = self.current_template_call_at_cursor() else {
            self.status_message = "No template call on this line".into();
            return Ok(());
        };
    ...
    pub(super) fn adjust_selected_template_call_repeats(&mut self, delta: i32) -> Result<()> {
    ...
    pub(super) fn adjust_current_template_call_time_scale(&mut self, delta: i32) -> Result<()> {
        let Some(call) = self.current_template_call_at_cursor() else {
            self.status_message = "No template call on this line".into();
            return Ok(());
        };
    ...
    pub(super) fn adjust_selected_template_call_time_scales(&mut self, delta: i32) -> Result<()> {
        let selected_indices = self.selected_template_call_indices();
        let mut selected = self.selected_template_call_spans();
        if selected.is_empty() {
            self.status_message = "No template call selected".into();
...
fn template_call_text_with_repeat_delta(raw_text: &str, delta: i32) -> Option<String> {
    let mut parsed = parse_template_call_editable(raw_text)?;
    parsed.repeat = adjust_template_call_count(parsed.repeat, delta);
    Some(parsed.to_text())
}

fn template_call_text_with_time_scale_delta(raw_text: &str, delta: i32) -> Option<String> {
    let mut parsed = parse_template_call_editable(raw_text)?;
...
fn parse_template_call_repeat(input: &str) -> Option<u32> {
    input
        .strip_prefix('x')
...
fn parse_template_call_time_scale(input: &str) -> Option<u32> {
    input
        .strip_prefix('/')
        .and_then(|digits| digits.parse::<u32>().ok())
        .filter(|value| *value >= 1)
...
fn format_template_call_repeat(value: u32) -> String {
    format!("x{}", value)
}

fn format_template_call_time_scale(value: u32) -> String {
    format!("/{}", value)
}
...
mod tests {
    use super::{
        ...
        parse_template_call_transpose, shrinkable_group_at_token, subdivided_unit,
        template_call_text_with_repeat_delta, template_call_text_with_time_scale_delta,
        transposed_template_call_text,
    };
