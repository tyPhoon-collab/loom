...
### フォーマット (Auto-formatting)
Loomは保存時にコードを自動整形します。以下のルールが適用されます。

1. Space Separation: `^`, `.`, `-` などの記号は、可読性のため必ずスペースで区切られます。
   - `c3|^..|` → `c3 | ^ . . |`
2. Block Alignment: 小節線 `|` の位置を、全トラックで最も長い行に合わせて縦に揃えます。
3. Note Sorting: トラック（行）を音の高さ順（High -> Low）に並べ替えます。
   - 行内の複数のノート（マルチノート）は低い順にソート。
   - 各行（トラック）は、その行に含まれる最も高い音を基準に、高音から低音へソート。
   - ドラムの場合もMIDIノート番号順（例: HiHat -> Snare -> Kick）に並びます。
4. Preservation: `# Header` や `> Comment` はそのまま維持されます。
